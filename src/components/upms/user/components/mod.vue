  ...
  <formPage descTitle="修改用户" @ok="handleSure" @cancel="$emit('close')">
    <el-form ref="modForm" :model="modData" :rules="rules" label-width="90px">
      ...
      <el-form-item class="be-depart" :prop="modData.companyId==10001?'beDepart':''" label="所属部门">
        ...
        <el-select style="width:400px;" disabled filterable multiple @change="changebeDepart" v-model="modData.beDepart">
      ...
      <el-form-item label="管理部门">
        <el-select style="width:400px;" filterable multiple v-model="modData.maDepart">
          <el-option v-for="item in selDep" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item prop="role" label="用户角色">
        <el-select style="width:400px;" @change="changeUserRole" filterable multiple v-model="modData.role">
          <el-option v-for="item in uMRole" :key="item.id" :label="item.roleName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="可分配角色">
        <el-select style="width:400px;" filterable multiple v-model="modData.asRole">
...
export default {
  ...
  methods: {
    ...
    async runmod() {
      ...
      const res = await this.$upms.updateUser(data);
      if (res) {
        this.$message({
          message: "提交成功",
          type: "success"
    ...
    selDepart() {
      ...
      this.changebeDepart(this.modData.beDepart);
    },
    getSelDep() {
    ...
    changebeDepart(list) {
      this.modData.maDepart = this.modData.maDepart.filter(item => {
        return list.includes(item);
      });
    },
    changeUserRole(list) {
      this.modData.asRole = this.modData.asRole.filter(item => {
        return list.includes(item);
      });
    }
  }
