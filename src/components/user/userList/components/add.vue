  ...
  <formPage descTitle="新增用户" @ok="handleSure" @cancel="$emit('close')">
    <el-form ref="addForm" :model="addData" :rules="rules" label-width="90px">
      ...
      <el-form-item :prop="addData.companyId==10001?'beDepart':''" label="所属部门">
        <span v-if="addData.companyId==10001" style="color:#f56c6c;position:absolute;top:0;left:-78px">*</span>
        <el-select style="width:300px" disabled filterable multiple @change="addData.maDepart=[]" v-model="addData.beDepart">
        ...
        <div v-if="addData.companyId==10001?showV:false" style="line-height:1px;margin-top:8px;margin-left:3px">
          <span style="color: #f56c6c;font-size: 12px;">必选</span>
        </div>
      </el-form-item>
      <el-dialog title="所属部门" :visible.sync="treeDialogShow" width="500px">
        <el-tree ref="tree" :data="allDep" check-strictly show-checkbox node-key="id" :props="defaultProps">
        </el-tree>
      ...
      <el-form-item label="管理部门">
        <el-select multiple v-model="addData.maDepart">
          <el-option v-for="item in selDep" :key="item.id" :label="item.name" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      ...
      <el-form-item prop="role" label="用户角色">
        <el-select @change="addData.asRole=[]" filterable multiple v-model="addData.role">
          <el-option v-for="item in uMRole" :key="item.id" :label="item.roleName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="可分配角色">
        <el-select multiple v-model="addData.asRole">
          <el-option v-for="item in setAsRole" :key="item.id" filterable :label="item.roleName" :value="item.id"></el-option>
        </el-select>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async runAdd() {
      const res = await this.$users.addUser(this.addData);
      if (res) {
        this.$message({
          message: "提交成功",
          type: "success"
