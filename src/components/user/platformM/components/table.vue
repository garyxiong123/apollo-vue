<template>
  <div class="table-box">
    <el-table v-loading="loading" :style="{border:'1px solid #ebeef5'}" :data="tableData" :max-height="tableMaxHeight">
      <el-table-column prop="backgroundName" label="后台名称" min-width="250">
      </el-table-column>
      <el-table-column prop="code" label="code" min-width="150">
      </el-table-column>
      <el-table-column prop="url" label="URL地址" min-width="250">
      </el-table-column>
      <el-table-column prop="departmentNames" label="所属部门" min-width="250">
      </el-table-column>
      ...
      <el-table-column fixed="right" label="操作" min-width="300">
        <template slot-scope="scope">
          <el-button @click.native.prevent="editShow(scope.row)" type="text" size="small">
            编辑
          </el-button>
          <el-button @click.native.prevent="del(scope.row)" style="color:red" type="text" size="small">
            删除
          </el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async delRun(id) {
      const res = await this.$users.delBack(id);
      if (res) {
        this.$message({
          type: "success",
          message: "删除成功!"
