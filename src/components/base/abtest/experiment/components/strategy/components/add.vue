  ...
  <div class="box">
    ...
    <el-form style="margin-top:20px" ref="form" :rules="rules" :model="form" label-width="80px">
      ...
      <el-form-item prop="strategyKey" label="key">
        <el-input v-model="form.strategyKey" style="width:400px"></el-input>
      </el-form-item>
      <el-form-item prop="description" label="描述">
        <el-input v-model="form.description" style="width:400px"></el-input>
      </el-form-item>
      <el-form-item prop="numberOrUploadCode" v-if="strategyType==1" label="百分比">
        <el-input-number v-model="form.numberOrUploadCode" :min="1" :max="100"></el-input-number>
      </el-form-item>
      <el-form-item prop="numberOrUploadCode" v-else-if="strategyType==2" label="指定设备">
        <Upload v-model="form.numberOrUploadCode"></Upload>
      </el-form-item>
      <el-form-item prop="numberOrUploadCode" v-else label="指定用户">
        <Upload v-model="form.numberOrUploadCode"></Upload>
      </el-form-item>
    </el-form>
    <div class="operbar">
...
export default {
  data() {
    return {
      ...
      rules: {
        ...
        numberOrUploadCode: { required: true, message: "必传", trigger: "blur" }
      },
      form: {},
      loading: false
    };
  ...
  created() {
    if (this.strategyType == 1) {
      this.$set(this.form, "numberOrUploadCode", 1);
    }
  },
  methods: {
    handleSure() {
    ...
    async handleRun() {
      this.loading = true;
      const res = await this.$base.addStrategy(
        ObjToFormData({ ...this.form, experimentId: this.experimentId })
      );
      this.loading = false;
      if (res) {
        this.$message({
