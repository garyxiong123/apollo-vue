<template>
  <div class="table-box">
    <el-table v-loading="loading" :style="{border:'1px solid #ebeef5'}" :data="tableData" :max-height="tableMaxHeight">
      <el-table-column prop="appName" label="app" min-width="200">
      </el-table-column>
      ...
      <el-table-column prop="sendTime" label="发送时间" min-width="250">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    toDetail(data) {
      sessionStorage.setItem("nowEmail", JSON.stringify(data));
      this.$router.push("/base/email/detail");
    }
  }
};
</script>
