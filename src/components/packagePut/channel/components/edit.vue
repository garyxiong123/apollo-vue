...
export default {
  ...
  mounted() {
    this.form = JSON.parse(sessionStorage.getItem("nowChannel"));
  },
  methods: {
    init() {
    ...
    async runAdd() {
      this.loading = true;
      const { id, packageVersionId, channelCode, tabType } = this.form;
      const data = { id, packageVersionId, channelCode, tabType };
      const res = await this.$base.updatePackageChannel(data);
      this.loading = false;
      if (res) {
        this.$message({
          type: "success",
