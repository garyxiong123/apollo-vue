...
export default class USER extends config {
  ...
  //token
  static addAppToken(data) {
    const path = `/${publicPath}/appToken/insert`;
    return this.comPost(path, data);
  }
  static getAppToken(id,code){
    const path = `/${publicPath}/appToken/list?appId=${id}&appCode=${code}`;
    return this.comGet(path);
  }
  static delAppToken(id){
    const path = `/${publicPath}/appToken/delete?appTokenId=${id}`;
    return this.comDel(path);
  }
}
